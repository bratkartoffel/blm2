...
if (CheckGameLock()) {        // Ist das Spiel gesperrt? Wenn ja, dann
    ...
    echo 'Die aktuelle Runde läuft seit dem <b>' . date("d.m.Y", LAST_RESET) . '</b> und dauert somit <b>bis zum ' . date("d.m.Y \u\m H:i", LAST_RESET + RUNDEN_DAUER) . '.</b><br />
					Der Erstplatzierte ist im Moment <b>' . htmlentities($db->getPlayerNameByRank(1), ENT_QUOTES, "UTF-8") . '</b>, 
					gefolgt von <b>' . htmlentities($db->getPlayerNameByRank(2), ENT_QUOTES, "UTF-8") . '</b> und 
					<b>' . htmlentities($db->getPlayerNameByRank(3), ENT_QUOTES, "UTF-8") . '.</b>';
}
