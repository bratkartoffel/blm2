...
<div class="form AdminEditContract">
    <form action="/actions/admin_vertrag.php" method="post">
        ...
        <header>Vertrag bearbeiten</header>
        <div>
            <label for="von">Absender:</label>
            <span><?= createDropdown(Database::getInstance()->getAllPlayerIdsAndName(), $entry['VonId'], 'von', false, true); ?></span>
        </div>
        <div>
            <label for="an">Empfänger:</label>
            <span><?= createDropdown(Database::getInstance()->getAllPlayerIdsAndName(), $entry['AnId'], 'an', false); ?></span>
        </div>
        <div>
