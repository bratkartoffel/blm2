...
<table id="SeitenUeberschrift">
	<tr>
		...
		<td style="font-size: 16pt; font-weight: bold; text-decoration: underline">Gruppe
			<a href="./?p=hilfe&amp;mod=1&amp;cat=23"><img src="pics/help.gif" alt="Hilfe" style="border: none;" /></a>
		</td>
	</tr>
</table>
<?php
	...
	if($id>0) {
			...
			<tr>
				<td>
					<div style="text-align: center;">
						<img src="pics/gruppe.php?id=<?=$gruppe->ID; ?>&amp;<?=intval(time()); ?>" alt="Hier k&ouml;nnte das Gruppenbild sein..." />
					</div>
					<div style="margin-top: 30px;">
						<?php
							if($gruppe->Beschreibung != NULL) {
								echo ReplaceBBCode(stripslashes($gruppe->Beschreibung), 90);
							}
							else {
				...
				<td style="width: 300px; border-left: 1px dashed #444444; text-align: center;">
					...
					<ul style="text-align: left; margin-left: 20px;">
						<?php
							...
							while($mitglied=mysql_fetch_object($sql_ergebnis)) {
								...
								echo '<a href="./?p=profil&amp;uid=' . $mitglied->ID . '&amp;' . intval(time()) . '">' . htmlentities(stripslashes($mitglied->Name), ENT_QUOTES, "UTF-8") . " (" . intval($mitglied->Punkte) . ")</a></li>";
					...
					<?php
						...
						if(mysql_num_rows($sql_ergebnis) > 0) {
							...
							while($nap=mysql_fetch_assoc($sql_ergebnis)) {
								echo '<li><a href="./?p=gruppe&amp;id=' . $nap["An"] . '&amp;' . intval(time()) . '">' . htmlentities(stripslashes($nap["Name"]), ENT_QUOTES, "UTF-8") . '</a></li>';
							}
							echo '</ul>';
						}
						else {
					...
					<?php
						...
						if(mysql_num_rows($sql_ergebnis) > 0) {
							...
							while($bnd=mysql_fetch_assoc($sql_ergebnis)) {
								echo '<li><a href="./?p=gruppe&amp;id=' . $bnd["An"] . '&amp;' . intval(time()) . '">' . htmlentities(stripslashes($bnd["Name"]), ENT_QUOTES, "UTF-8") . '</a></li>';
							}
							echo '</ul>';
						}
						else {
					...
					<?php
						...
						if(mysql_num_rows($sql_ergebnis) > 0) {
							...
							while($krieg=mysql_fetch_assoc($sql_ergebnis)) {
								echo '<li><a href="./?p=gruppe&amp;id=' . $krieg["An"] . '&amp;' . intval(time()) . '">' . htmlentities(stripslashes($krieg["Name"]), ENT_QUOTES, "UTF-8") . '</a></li>';
							}
							echo '</ul>';
						}
						else {
					...
					<form action="actions/gruppe.php" method="post">
						...
						<input type="submit" value="Gruppe verlassen" onclick="return confirm('Wollen Sie diese Gruppe wirklich verlassen?');" />
					</form>
					<?php
						}
						else {
		...
		<form action="actions/gruppe.php" method="post">
			...
			<table class="Liste" style="width: 400px; margin-left: 50px; margin-top: 20px;">
				...
				<tr>
					<td style="text-align: center;">
						<input type="submit" value="Schreiben" onclick="document.forms[1].submit(); this.disabled='disabled'; this.value='Bitte warten...'; return false;" />
					</td>
				</tr>
			</table>
		</form>
		<?php
			...
			while($nachricht=mysql_fetch_object($sql_ergebnis)) {
				...
				echo 'Von <b><a href="./?p=profil&amp;uid=' . $nachricht->mID . '&amp;' . time() . '">' . htmlentities(stripslashes($nachricht->Name), ENT_QUOTES, "UTF-8") . '</a></b>
								...
								<tr>
									<td>' . ReplaceBBCode(stripslashes($nachricht->Nachricht), 55) . '</td>
								</tr>
							</table>';
			}
				...
				for($i=0; $i<$anzahl_nachrichten; $i++) {		// so, dann gehen wiŕ mal alle Spieler durch
					if($i%GRUPPE_OFFSET == 0) {									// Wenn wir gerade bei einem "Offset-Punkte" angekommen sind, dann...
						if(($i/GRUPPE_OFFSET) != $offset) {					// Wenn der gerade bearbeitende Offset nicht der angefordete ist, dann...
							$temp.='<a href="./?p=gruppe&amp;o=' . ($i/GRUPPE_OFFSET) . '&amp;id=' . $gruppe->ID . '&amp;' . intval(time()) . '">' . (($i/GRUPPE_OFFSET)+1) . '</a> | ';	// Zeig die Nummer des Offsets als Link an
						}
						else {
		...
		<form action="actions/gruppe.php" method="post">
			...
			<table class="Liste" cellspacing="0" style="width: 300px;">
				...
				<tr>
					<td>Name:</td>
					<td><input type="text" name="name" maxlength="32" value="<?=htmlentities(stripslashes($_GET['gruppe']), ENT_QUOTES, "UTF-8"); ?>" /></td>
				</tr>
				<tr>
					<td>Beitrittspasswort:</td>
