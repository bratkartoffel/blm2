...
?><table id="SeitenUeberschrift">
	<tr>
		...
		<td style="font-size: 16pt; font-weight: bold; text-decoration: underline">Die Plantage
			<a href="./?p=hilfe&amp;mod=1&amp;cat=5"><img src="pics/help.gif" alt="Hilfe" style="border: none;" /></a>
		</td>
	</tr>
</table>
<?php
...
?>
<b>
	Hier k&ouml;nnen Sie Ihre erforschten Obst- und Waresorten anbauen.<br />
</b>
<br />
		...
		<tr>
			<td style="font-weight: bold; padding: 5px;">
				...
				<input type="submit" value="Abschicken" disabled="disabled" />
						...
						?> 
				<input type="submit" value="Abschicken" onclick="document.forms[0].submit(); this.disabled='disabled'; this.value='Bitte warten...'; return false;" />
						<?php
					}
				?> 
			</td>
	...
	<tr>
		...
		<td>
			<p>
				...
				<b><?=number_format(($menge/date("H", $dauer))/60, 3, ",", "."); ?> kg / Minute</b><br />
				...
				<form action="actions/plantage.php" method="post" name="pr_<?=$i; ?>">
					...
					<?php
						...
						if(intval($auftraege->$temp->ID) > 0) {		// Wenn der Auftrag schon gegeben wurde, dann...
							...
							echo '<i>Es l&auml;uft bereits ein Anbau!</i><br />
										(noch ' . date("H:i:s", $auftraege->$temp->Start+$auftraege->$temp->Dauer-time()-3600) . ' verbleibend.)<br />
										<a onclick="return confirm(\'Wollen Sie den Auftrag wirklich abbrechen? Sie bekommen die Kosten nicht zur&uuml;ck erstattet, lediglich die bisher produzierte Menge (~ ' . intval($auftraege->$temp->Menge*$ProzentFertig) . ' kg) wird Ihnen gut geschrieben.!\');" href="actions/auftrag.php?a=1&amp;id=' . $auftraege->$temp->ID . '&amp;back=plantage&amp;was=' . $i . '">Abbrechen</a>';
						}
