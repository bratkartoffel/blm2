...
public class MailingTests extends AbstractTest {
    ...
    @BeforeEach
    void beforeEach() {
        getDriver().get("http://%s/actions/logout.php".formatted(LOCALHOST));
    }
    ...
    void testRegistration() {
        ...
        Assertions.assertTrue(body.contains("http://%s/actions/activate.php?user=%s&code=%s".formatted(LOCALHOST, username, RANDOM_TOKEN)));
    }
    ...
    void testChangeEmailAdress(TestInfo testInfo) {
        ...
        String body = ((Map<String, String>) mail.get("body")).get("text");
        Assertions.assertTrue(body.contains("http://%s/actions/activate.php?email=%s&code=%s".formatted(LOCALHOST, URLEncoder.encode(email, StandardCharsets.UTF_8), RANDOM_TOKEN)));
    }
    ...
    void testPasswordReset(TestInfo testInfo) {
        ...
        String body = ((Map<String, String>) mail.get("body")).get("text");
        Assertions.assertTrue(body.contains("http://%s/actions/pwd_reset.php?a=2&id=%d&token=%s".formatted(LOCALHOST, userId3, RANDOM_TOKEN)));
    }
    ...
    void testPasswordRecovery(TestInfo testInfo) {
        ...
        WebDriver driver = getDriver();
        driver.get("http://%s/actions/pwd_reset.php?a=2&id=%d&token=%s".formatted(LOCALHOST, userId4, RANDOM_TOKEN));
        ...
        String body = ((Map<String, String>) mail.get("body")).get("text");
        Assertions.assertTrue(body.contains("changeit"));
