...
*/
	...
	**/
	...
	/**
		* Hilfsfunktion: Liefert die Platzierung des Spiels auf 3 Vote-Seiten als Assoziatives Array zurück
		* 
		* @author Simon Frankenberger <simonfrankenberger@web.de>
		* @version 1.0.0
		* 
		* @return array
	**/
	function getPlatzierung() {
		$datei=fopen("votes/galaxy-news.txt", "r");
		$daten=fgets($datei, 1024);
		
		$platz["galaxynews.de"]=$daten;
		fclose($datei);
		
		
		$datei=fopen("votes/gamesdynamite.txt", "r");
		$daten=fgets($datei, 1024);
		
		$platz["gamesdynamite.de"]=$daten;
		fclose($datei);
		
		
		$datei=fopen("votes/browserwelten.txt", "r");
		$daten=fgets($datei, 1024);
		
		$platz["browserwelten.de"]=$daten;
		fclose($datei);
...
[i]-System-[/i]";		// die Nachricht mit den 5 Erstplatzierten an alle Spieler
	...
	/**
		* Hilfsfunktion: Gibt zurück, ob das Banner angezeigt werden soll, oder nicht
		* 
		* @author Simon Frankenberger <simonfrankenberger@web.de>
		* @version 1.0.0
		* 
		...
		* @return boolean
	**/
	function ShowBanner($lastView) {
		return ($lastView+BANNER_LOCKTIME < time());
	}
