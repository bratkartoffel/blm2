...
function toggleRundmail(){let a=document.getElementById("receiver"),b=document.getElementById("broadcast");"0"===b.value?(a.value="RUNDMAIL",a.disabled="disabled",b.value="1"):(a.value="",a.disabled="",a.enabled="enabled",b.value="0");return!1}function enableSitterOptions(a){Array.prototype.forEach.call(document.getElementById("sitterSettings").getElementsByTagName("input"),b=>{"sitting_aktiviert"!==b.id&&"submit"!==b.type&&"hidden"!==b.type&&(b.disabled=!a)})}
function enableImportOptions(a){document.getElementById("verify").disabled=a;document.getElementById("ignore_round").disabled=a}function chefboxPollJobs(){opener&&window.setInterval(()=>{var a=opener.document.getElementsByClassName("MessageBox");0!==a.length&&(a=a[0],a.hasAttribute("reload-chefbox")&&(a.removeAttribute("reload-chefbox"),window.location.reload()))},1E3)}
...
function setupNachrichtSchreiben(){let a=document.getElementById("toggle_rundmail");null!==a&&(a.onclick=()=>toggleRundmail())}setupNachrichtSchreiben();
