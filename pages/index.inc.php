...
try {
    ...
    $error = true;
    ?>
    <h2 style="color: red">Ungültige Installation</h2>
    <p>
        Es konnte keine Datenbankverbindung hergestellt werden, bitte überprüfe deine <code>config.inc.php</code>.
    </p>
    <?php
}
if (!$error && !Database::getInstance()->tableExists('mitglieder')) {
    $error = true;
    ?>
    <h2 style="color: red">Ungültige Installation</h2>
    <p>
        Die Installation das Spiels ist fehlerhaft. Bitte überprüfe deine <code>config.inc.php</code> und führe das
        <a href="/install/update.php?secret=__HIER_SECRET_EINTRAGEN__" target="_blank">update-Script</a> (mit deinem
        gewählten Secret)
        aus.
    </p>
    <?php
